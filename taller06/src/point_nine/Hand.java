...
public class Hand {
  ...
  private void setIsBusted() {
    ...
    if (score > 21) {
      this.isBusted = true;
      if (convertAcesToOneScore()) { // Si se convirtieron algun Ace, se vuelve a verificar los puntos de la mano.
        setIsBusted();
      }
    } else
      this.isBusted = false;
  ...
  /**
   * Convierte los Aces de 11 puntos a 1 punto.
   * 
   * @return true si se convirtio algun Ace.
   * @return false si no se convirtio algun Ace.
   */
  private boolean convertAcesToOneScore() {
    ...
    for (int i = 0; i < cards; i++) {
      isAceConverted = cardArray[i].getPointAce() == 11; // Verifica si el Ace es 11 puntos.
      cardArray[i].setPointAce(); // Convierte el Ace a 1 punto.

      if (isAceConverted) { // Si algun Ace fue convertido, se vuelve true.
        response = true;
      }
    }
